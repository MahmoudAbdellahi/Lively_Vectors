...
switch classifier_name
    ...
    case 'lda'
        ...
        % feature selection
        % % cfgmi=[]; cfgmi.data = TRAIN; cfgmi.labels = TRAIN_GROUP;
        % % mi = lv_mutual_information(cfgmi);
        % % [~, id] = sort(mi, 'descend');
        % % TRAIN = TRAIN(:,id(1:5)); TEST = TEST(:,id(1:5));
...
% to control the smoothing of every distribution (the higher the more smoothing) and when we have
% a new point we project it into the distribution of each point and then take the average of that .. in matlab
