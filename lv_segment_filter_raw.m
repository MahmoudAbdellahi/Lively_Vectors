function [ segmented_data ] = lv_segment_filter_raw(sbj,type, part, sleep_stage,ref_ch ,rawdir, pre_stim,post_stim, required_sampling_rate)
% addDirs .. passing rawdir and mri append is removed
...
if strcmp(type,'img')~=1 % imagery is in one part
    ...
    typeEEG = ['part' num2str(sbj) '_' typeEEG]; rawdir = 'E:\MRI_jittered_exp\allIMG';% [datadir '/RawData/allImagery'];
end
...
for i=1: length(hdr.label)
    ...
    % the 0.1 high pass could help to remove the slow drifts that affect
    % all signals and also demean the data because it would remove 0hz
    % which is the mean of data that affects all of them the same
    cfg.bsfilter  = 'yes'; % band-stop method
    cfg.bsfreq    = [48 52];
    % cfg.demean      = 'yes'; because 0hz was removed in the bandpass
    % filtering
...
%% create layout .. would be needed when doing interpolation and then doing the plotting of erp
% and tf on the topos.
segmented_data = lv_create_layout(segmented_data, sbj);


%% sleep scoring ... assuming done and that the stimuli were in the correct sleep stage
...
% may not include for now because it will be saved and then loaded for
% every ppnt so should be changing and that should be fine
