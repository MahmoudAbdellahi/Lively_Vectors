...
if length(size(data.trial))==3 %rpt_ch_time
    if isfield(data,'trialinfo')
        ...
        for i=1:size(data.trial,2)
            label_size_factor = 0.08 - ((0.08 - 0.04) / (58 - 6)) * (length(pos) - 6); % to have the figures proportional to the no. channels
            axes('Position',[pos(i,1) pos(i,2)  label_size_factor label_size_factor]);
            % axes('Position',[pos(i,1) pos(i,2)  0.05 0.05]) % the first two for the position of the figure if it is from 0 to 1 and then the position of the figure
        ...
        if cluster_in_space==0
            for i=1:size(data.trial,2)
                % axes('Position',[pos(i,1) pos(i,2)  0.05 0.05])
                label_size_factor = 0.08 - ((0.08 - 0.04) / (58 - 6)) * (length(pos) - 6); % to have the figures proportional to the no. channels
                axes('Position',[pos(i,1) pos(i,2)  label_size_factor label_size_factor]);
                if do_stats==0
...
if isfield(info.data,'powspctrm')
    ...
    if size(data.powspctrm,1)>2 % group lvl
        if info.do_stats == 1 
            ...
            % it would the difference of a difference so would be zeros
        end
    else
...
if cluster_in_space==0
    ...
    temp.cond1 = hold_cond1S_powspctrm; temp.cond2 = hold_cond2S_powspctrm;
    if size(temp.cond1,1) > 3
        temp.cond1 = squeeze(mean(temp.cond1, 1));
        temp.cond2 = squeeze(mean(temp.cond2, 1));
        if length(size(temp.cond1))== 2 % because plot 2d expects first dime to be channels so if we 
        % repeat on topo we would have one channel which is the average
            tempcond1(1,:,:) = temp.cond1;
            tempcond2(1,:,:) = temp.cond2;
            temp.cond1 = tempcond1;
            temp.cond2 = tempcond2;
        end 
    end
...
if isequal(class(chance), 'cell'), chance=nan; end
...
for i=1:size(cond1S,2) % on every channel
    ...
    [statsStruct,zval] = lv_mdim_clusterstats(cfg, Diffresult, 0);
    
    % MVPA stats
    % addpath(genpath(fullfile(pwd, 'MVPA-Light-master')))
...
if size(tfr.powspctrm,1)>2 % two conditions and more than one ppnt
    odds = 1:2:size(tfr.powspctrm,1);
    even = 2:2:size(tfr.powspctrm,1);
    tfr.powspctrm(1, :,:,:) = mean(tfr.powspctrm(odds, :,:,:), 1);
    tfr.powspctrm(2, :,:,:) = mean(tfr.powspctrm(even, :,:,:), 1);
    tfr.powspctrm(3:end,:,:,:) = [];
end

cond1 = squeeze(tfr.powspctrm(1, :,:,:)); % sbj(cond)_ch_freq_time
cond2 = squeeze(tfr.powspctrm(2, :,:,:));
% getting the difference
temp = tfr;  temp.powspctrm=[];
...
temp.chance = 0; % assumed chance for difference
plot2d(temp ,'Difference (c1-c2)',pos, [] );
...
% cond2
% making sure that the dimensions are channels and then two other dimensions that will be plotted in 2d
if length(size(data_struct.powspctrm))~=3 || length(size(data_struct.cond1))~=3 || length(size(data_struct.cond2))~=3
   error('make sure that data is channelsxdim1xdim2 (dim1 and dim2 should be the 2d plot)') 
end
...
for i=1:size(data,1)
    if size(data,1)>1 % many figures .. many figure takes code=100 and single figure (one channel) code=1000
        ...
        label_size_factor = 0.08 - ((0.08 - 0.04) / (58 - 6)) * (length(pos) - 6); % to have the figures proportional to the no. channels
        figure(fig_code), axes('Position',[pos(i,1) pos(i,2)  label_size_factor label_size_factor]);
        figure(fig_code+1), axes('Position',[pos(i,1) pos(i,2)  label_size_factor label_size_factor]);
        figure(fig_code+2), axes('Position',[pos(i,1) pos(i,2)  label_size_factor label_size_factor]);
    end
...
if size(data,1)>1 % many channels and many figures
    ...
    % colorbar_position = [0.88 0.05 0.2 0.2]; % horizonal_position bottom_edge width height
    % c = colorbar('peer',axes('Position',colorbar_position));
    % caxis(limits);
    % c.Label.String = 'condition 1'; axis('off');  c.Label.Position(1) = -2; grid on
    colorbar_position = [0.4 0.07 0.2 0.1]; % horizonal_position bottom_edge width height
    c = colorbar('peer', axes('Position', colorbar_position), 'Location', 'southoutside');
    caxis(limits);
    c.Label.String = colorlabel; axis('off'); c.Label.Position = [0 3.1 0]; c.FontSize = 8; grid on
    
    figure(fig_code+1),
    % c = colorbar('peer',axes('Position',colorbar_position));
    % caxis(limits_conditions);
    % c.Label.String = 'condition 2'; axis('off');  c.Label.Position(1) = -2; grid on
    c = colorbar('peer', axes('Position', colorbar_position), 'Location', 'southoutside');
    caxis(limits_conditions);
    c.Label.String = 'condition 1'; axis('off'); c.Label.Position = [0 3.1 0]; c.FontSize = 8; grid on
    ...
    c = colorbar('peer', axes('Position', colorbar_position), 'Location', 'southoutside');
    caxis(limits_conditions);
    c.Label.String = 'condition 2'; axis('off'); c.Label.Position = [0 3.1 0]; c.FontSize = 8; grid on
    
else
    figure(fig_code)
    h = colorbar; title('comparison title'); title(h, data_struct.label(i));
    xlabel('x label'), ylabel('y label'); grid on
    figure(fig_code+1)
    h = colorbar; title(h,'clabel'); title('condition 1');
    xlabel('x label'), ylabel('y label'); grid on
    figure(fig_code+2)
    h = colorbar; title(h,'clabel'); title('condition 2');
    xlabel('x label'), ylabel('y label'); grid on
end
