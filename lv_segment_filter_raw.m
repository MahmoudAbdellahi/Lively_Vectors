...
% loading data one channel at a time to save memory
for i=1: length(hdr.label)
    %% Filtering
    waitbar(i/length(hdr.label), b, ['Segmenting and filtering data, please wait ... channel: ' hdr.label(i)],'Interpreter','none','WindowStyle','docked');
    cfg             = [];
    cfg.dataset     = data_raw;
    cfg.channel = i;
