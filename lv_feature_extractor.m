...
switch cfg.method
    case {'phase','power'} % hilbert at every time pt .. be careful of edges
        ...
        if strcmp(cfg.method,'power')==1
            ...
            % features_data.trial = phase_vals;
        end
    ...
    case 'spectrum'
        ...
        dat = data.trial;
        nyquistVar = cfg.sampling_rate/2; 
        N = size(dat,3);
        pts_hz = linspace(0,nyquistVar,(N/2)+1);
        coeff = fft(dat,[],3);
        magnitude = abs(coeff(:,:,1:length(pts_hz)))*2;
        delta_f = cfg.sampling_rate/N;  % frequency resolution
        features_data.trial = (magnitude.^2)/(N*delta_f);  % Now it's PSD in units²/Hz .. 
        features_data.pts_hz = pts_hz;
    case 'fourier_series'
        % fitting fourier series to the input signals
        % function [coeffs, R_squared, fit_obj]= fitting_fourier(nn, t, d)
        % each time a fourier series at a particular frequency is used and
        % the optimisation would change the amplitude and phase of that
        % signal so that it fits the data with min error .. then we could
        % measure the goodness of fit Rsquared and also the fitted signal
        % itself is the fit_obj .. Ho_sensitivity_decision... has some
        % parts on plotting the signals and getting the phase compared to
        % bootstrapping and amplitude spectrum chance level
        t = data.time; 
        for trl=1:size(data.trial,1)
            d = data.trial(trl,:); % assuming 2d of trialxtime so the fit for every trial
            c=0;
            fit_obj=[];
            for fixedFrequency = 4:0.1:10  % these are the frequencies tested
                c=c+1;
                fixedFrequencyW = fixedFrequency * 2*pi; % to Hz because w in the equation of fit is 2pi*f
                f = fittype('fourier1');
                startPoint = [mean(d), std(d), std(d)]; % any assumed starting point
                % Create options for the fit
                opts = fitoptions(f);
                opts.StartPoint = [startPoint, fixedFrequencyW]; % [a0, a1, b1, w] .. so w is fixed now because the start and lower and upper are the same value
                opts.Lower = [-Inf, -Inf, -Inf, fixedFrequencyW];
                opts.Upper = [Inf, Inf, Inf, fixedFrequencyW];
                opts.MaxIter = 400;
                %opts.Method % they used NonlinearLeastSquares and it should be here

                % Fit the model to your data
                fitobject = fit(t, d, f, opts);
                coeffs(c, :) = coeffvalues(fitobject);
                % gof = goodnessOfFit(fitobject(t), d, 'NMSE') % Mean Squared Error
                % plot(fitobject,t,d)
                ...
                SSR = sum(residuals.^2);
                SST = sum((d - mean(d)).^2);
                R_squared(1, c) =  (1-(SSR/SST));

                fit_obj{1,c} = fitobject;
            end
        end
        features_data.fit_obj = fit_obj;
        features_data.R_squared = R_squared;
        features_data.coeffs = coeffs;
