...
% would leave the part before that so that if there is a potential bad
% channel then we try to inspect that at the end because bad channel is a
% big thing so we might do visual inspection and be sure before doing that
...
bad_segments = ( sum(bad_temp,1) > size(dat,1)*0.5 ); % 0.25 channels for more than 50ms .. yes fixed because that should help with inspection
CC = bwconncomp(bad_segments); win_s = (required_sampling_rate * 50)/1000; 
...
% cfg=[]; cfg.ylim=[-250 250]; % single trial based browsing
% artf=ft_databrowser(cfg,temp_data);
browser_cfg=[]; trls_to_show_together = 40; browser_cfg.ylim=[-250 250]; % continuous browsing
